...
Node* insertAtBeforeValue(Node *head, int ele, int val) {
	// Case when the head is NULL, meaning the list is empty
	if (head == nullptr) {
		return NULL;
	}
...
int main() {
	...
	// After inserting the element at given position
	head = insertAtPosition(head, 9, 9);
	cout << "After inserting the element at given position: ";
	print(head);

	// After inserting the element before the given value
	head = insertAtBeforeValue(head, 18, 9);
	cout << "After inserting the element before the given value: ";
	print(head);
}
